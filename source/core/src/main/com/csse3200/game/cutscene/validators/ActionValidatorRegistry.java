...
public class ActionValidatorRegistry {
     ...
     * @param context {@link ValidationCtx} object containing relevant information for performing validation
     * @return A list of {@link AuthoringError}s detailing any validation failures in the provided {@link ActionDTO}
     */
    public List<AuthoringError> validate(ActionDTO action, String beatId, ValidationCtx context) {
        if (action.type == null) return List.of(
                new AuthoringError("ACTION_TYPE_NULL", "doc.cutscene.beats." + beatId + "actions.*",
                        "Action type must be a string and not null"));
        else if (!actionValidators.containsKey(action.type)) return List.of(
                new AuthoringError("ACTION_TYPE_INVALID", "doc.cutscene.beats." + beatId + "actions.*",
