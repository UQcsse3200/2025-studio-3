...
public class ValidatorUtils {
     ...
     * <ul>
     *     <li>For each map entry {@link Map.Entry}:</li>
     *     <ul>
     *         <li>{@link Map.Entry#getKey()} must not be null</li>
     *         <li>{@link Map.Entry#getKey()} must not be empty</li>
     *         <li>{@link Map.Entry#getValue()} must not be null</li>
     *         <li>{@link Map.Entry#getValue()} must not be empty</li>
     *     </ul>
     * </ul>
     *
    ...
    /**
     * Takes a {@code beatId} string and {@link ActionDTO} and validates that a valid transition is present in the {@link ActionDTO}
     *
     * The conditions for a valid transition are as follows:
     * <ul>
     *     <li>The {@code transition} field value must be a string</li>
     *     <li>The {@code transition} field value must be in {@link Transition#displayNames()}</li>
     *     <li>The {@code duration} field value must be an integer</li>
     *     <li>The {@code duration} field value must be {@code >0}</li>
     * </ul>
     *
    ...
    public static List<AuthoringError> validateTransition(String beatId, ActionDTO action) {
        ...
        List<AuthoringError> transitionErrors = validateString(transitionObject, "transition",
        ...
        if (transitionErrors.isEmpty()){
            if (!Transition.displayNames().contains((String)transitionObject)) {
                errors.add(new AuthoringError("ACTION_TRANSITION_INVALID",
                        "doc.cutscene.beats." + beatId + ".actions.*",
        ...
        List<AuthoringError> durationErrors = validateInt(durationObject, "duration",
                "doc.cutscene.beats." + beatId + ".actions.*");
        ...
        if (durationErrors.isEmpty()) {
            if (((Long)durationObject).intValue() <= 0) {
                errors.add(new AuthoringError("ACTION_DURATION_INVALID",
                        "doc.cutscene.beats." + beatId + ".action.*", "Duration must be greater than 0"));
            }
        }
     ...
     * It checks that:
     * <ul>
     *     <li>The {@link Object} is not null</li>
     *     <li>The {@link Object} is an instance of {@link String}</li>
     * </ul>
     *
    ...
    public static List<AuthoringError> validateString(Object object, String key, String path) {
        if (object == null) {
            return List.of(new AuthoringError("ACTION_" + key.toUpperCase() + "_NULL",
                    path, key + " can not be null"));
        }

        if (!(object instanceof String)) {
            return List.of(new AuthoringError("ACTION_" + key.toUpperCase() + "_NOT_STRING",
     ...
     * <ul>
     *     <li>The {@link Object} is not null</li>
     *     <li>The {@link Object} is an instance of {@link Integer} or {@link Long}</li>
     *  </ul>
     *
     * @param object The alleged string object to be validated
     * @param key    The key for error (used for tracing)
    ...
    public static List<AuthoringError> validateInt(Object object, String key, String path) {
        ...
        if (!(object instanceof Integer) && !(object instanceof Long)) {
            return List.of(new AuthoringError("ACTION_" + key.toUpperCase() + "_NOT_INTEGER",
     ...
     * <ul>
     *     <li>The {@link Object} is not null</li>
     *     <li>The {@link Object} is an instance of {@link Float} or {@link Double}</li>
     *  </ul>
     *
     * @param object The alleged string object to be validated
     * @param key    The key for error (used for tracing)
    ...
    public static List<AuthoringError> validateFloatDouble(Object object, String key, String path) {
        ...
        if (!(object instanceof Float) && !(object instanceof Double)) {
            return List.of(new AuthoringError("ACTION_" + key.toUpperCase() + "_NOT_FLOAT",
                    path, key + " must be a float"));
        }
    ...
    /**
     * Validates a java {@link Object} to see if it's a valid {@link Float} or {@link Double}
     * If it is a valid {@link Float} or {@link Double}, it then converts it to a double and checks if it's in the
     * specified range.
     *
     * @param object The alleged string object to be validated
     * @param key    The key for error (used for tracing)
     ...
     * @return A list of {@link AuthoringError} detailing each rule infraction
     */
    public static List<AuthoringError> validateDoubleWithRange(Object object, String key, String path,
     ...
     * <ul>
     *     <li>The {@link Object} is not null</li>
     *     <li>The {@link Object} is an instance of {@link Boolean}</li>
     *  </ul>
     *
     * @param object The alleged string object to be validated
     * @param key    The key for error (used for tracing)
    ...
    public static List<AuthoringError> validateBool(Object object, String key, String path) {
        ...
        if (!(object instanceof Boolean)) {
            return List.of(new AuthoringError("ACTION_" + key.toUpperCase() + "_NOT_BOOLEAN",
    ...
    /**
     * Validates that a {@link Comparable} {@link Number} is within the inclusive range [{@code min}, {@code max}].
     *
    ...
    public static <N extends Number & Comparable<N>> List<AuthoringError> validateRange(
        ...
        if (number.compareTo(min) < 0) {
            return List.of(new AuthoringError("NUMBER_TOO_SMALL", path,
                    "The number " + number + " is smaller than the minimum value " + min));
        }

        if (number.compareTo(max) > 0) {
            return List.of(new AuthoringError("NUMBER_TOO_LARGE", path,
    ...
    /**
     * Takes a {@code beatId} string and {@link ActionDTO} and validates that a valid await is present in the {@link ActionDTO}
     * For a valid await, the {@link ActionDTO} must contain a valid boolean value under the tag {@code "await"}
     *
     * @param beatId The string id of the beat being processed
     * @param action The {@link ActionDTO} that the transition is contained in
     * @return A list of {@link AuthoringError} detailing each rule infraction
    ...
    public static List<AuthoringError> validateAwait(String beatId, ActionDTO action) {
        ...
        List<AuthoringError> awaitErrors = validateBool(awaitObject, "await",
                "doc.cutscene.beats." + beatId + ".actions.*");
    ...
    /**
     * Takes a {@code beatId} string and {@link ActionDTO} and validates that a valid sound ID is present in the {@link ActionDTO}
     * For a valid soundID, the {@link ActionDTO} must contain a valid string value under the tag {@code "await"} AND
     * the value of the field must be in the {@link ValidationCtx#soundIds()}.
     *
     * @param action  The {@link ActionDTO} that the transition is contained in
     ...
     * @return A list of {@link AuthoringError} detailing each rule infraction
     */
    public static List<AuthoringError> validateSoundId(ActionDTO action, ValidationCtx context,
                                                 String path) {
        List<AuthoringError> soundIdErrors = ValidatorUtils.validateString(action.fields.get("soundId"),
                "soundId", path);

        if (soundIdErrors.isEmpty()) {
            String soundId = (String) action.fields.get("soundId");
            if (!context.soundIds().contains(soundId)) {
                soundIdErrors.add(new AuthoringError("ACTION_AUDIO_PLAY_INVALID_SOUND_ID", path,
