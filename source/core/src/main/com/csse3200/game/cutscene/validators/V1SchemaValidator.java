...
public class V1SchemaValidator implements SchemaValidator {
     ...
     *     <li>The document has the correct schema version</li>
     *     <li>Characters exist and are valid</li>
     *     <li>Backgrounds exist and are valid</li>
     *     <li>Sounds exist and are valid</li>
     *     <li>Cutscenes exist and are valid</li>
     * </ul>
     *
    ...
    public List<AuthoringError> validate(CutsceneDocDTO cutsceneDocDTO) {
        ...
        if (cutsceneDocDTO == null) {
            return List.of(new AuthoringError("DOC_NULL", "doc",
                    "The json file is null or invalid."));
        }

        if (cutsceneDocDTO.schemaVersion != 1) {
            return List.of(new AuthoringError("INVALID_SCHEMA", "doc",
                    "The schema version is invalid, should be 1."));
        }

        if (cutsceneDocDTO.characters == null) {
            return List.of(new AuthoringError("CHARACTERS_NULL", "doc",
                    "Characters are required"));
        }

        if (cutsceneDocDTO.backgrounds == null) {
            return List.of(new AuthoringError("BACKGROUNDS_NULL", "doc",
                    "Backgrounds are required"));
        }

        if (cutsceneDocDTO.sounds == null) {
            return List.of(new AuthoringError("SOUNDS_NULL", "doc",
                    "Sounds are required"));
        }

        if (cutsceneDocDTO.cutscene == null) {
            return List.of(new AuthoringError("CUTSCENE_NULL", "doc",
                    "Cutscene is required"));
        }
     ...
     * <ul>
     *     <li>{@link CharacterDTO#id} must not be null</li>
     *     <li>{@link CharacterDTO#name} must not be null</li>
     *     <li>{@link CharacterDTO#poses} must not be null or empty</li>
     *     <li>Every entry in {@link CharacterDTO#poses} must have a non empty and non null {@link String} key and {@link String} value</li>
     *     <li>{@link CharacterDTO#id} must be unique across all characters</li>
     * </ul>
     *
    ...
    private List<AuthoringError> characterValidation(List<CharacterDTO> characters) {
        ...
        for (CharacterDTO characterDTO : characters) {
            if (characterDTO.id == null) characterErrors.add(new AuthoringError(
                    "NULL_CHARACTER_ID", "doc.characters", "Character id is null"));

            if (characterDTO.name == null) characterErrors.add(new AuthoringError(
                    "NULL_CHARACTER_NAME", "doc.characters", "Character name is null"));

            if (characterDTO.poses.isEmpty()) characterErrors.add(new AuthoringError(
                    "NULL_CHARACTER_EMPTY_POSES", "doc.characters." + characterDTO.id,
                    "Character must have at lest 1 pose"));
            else if (!ValidatorUtils.stringMapValid(characterDTO.poses)) {
                characterErrors.add(new AuthoringError(
                        "EMPTY_POSE", "doc.characters." + characterDTO.id,
            ...
            if (characterDTO.id != null) {
                if (characterIds.contains(characterDTO.id)) {
                    characterErrors.add(new AuthoringError(
                            "CHARACTER_ID_TAKEN", "doc.characters." + characterDTO.id,
     ...
     * <ul>
     *     <li>{@link BackgroundDTO#id} must not be null</li>
     *     <li>{@link BackgroundDTO#id} must be unique</li>
     *     <li>{@link BackgroundDTO#image} must not be null</li>
     * </ul>
     *
    ...
    private List<AuthoringError> backgroundValidation(List<BackgroundDTO> backgrounds) {
        ...
        for (BackgroundDTO background : backgrounds) {
            if (background.id == null) backgroundErrors.add(new AuthoringError(
                    "NULL_BACKGROUND_ID", "doc.backgrounds", "Background id is null."));

            if (background.image == null) backgroundErrors.add(new AuthoringError(
            ...
            if (background.id != null) {
                if (backgroundIds.contains(background.id)) {
                    backgroundErrors.add(new AuthoringError(
                            "BACKGROUND_ID_TAKEN", "doc.backgrounds." + background.id,
     ...
     * <ul>
     *     <li>{@link SoundDTO#id} must not be null</li>
     *     <li>{@link SoundDTO#file} must not be null</li>
     *     <li>If {@link SoundDTO#id} isn't null, it must correspond to a {@link SoundDTO}</li>
     * </ul>
     *
    ...
    private List<AuthoringError> soundValidation(List<SoundDTO> sounds) {
        ...
        for (SoundDTO sound : sounds) {
            if (sound.id == null) soundErrors.add(new AuthoringError(
                    "NULL_SOUND_ID", "doc.sounds", "Sound id is null."));

            if (sound.file == null) soundErrors.add(new AuthoringError(
                    "NULL_SOUND_FILE", "doc.sounds", "Sound file address is null"));

            if (sound.id != null) {
                if (soundIds.contains(sound.id)) {
                    soundErrors.add(new AuthoringError(
                            "SOUND_ID_TAKEN", "doc.sounds." + sound.id,
     ...
     * <ul>
     *     <li>{@link CutsceneDTO#id} must not be null</li>
     *     <li>{@link CutsceneDTO#beats} must not be null</li>
     *     <li>{@link CutsceneDTO#beats} must be a list of {@link BeatDTO}</li>
     *     <li>Each {@link BeatDTO} in {@link CutsceneDTO#beats} must have a valid {@link BeatDTO#id}</li>
     * </ul>
     * 
     * It also constructs a {@link ValidationCtx} to pass Cutscene information to the {@link V1SchemaValidator#beatValidation(BeatDTO, ValidationCtx)}
     *
    ...
    private List<AuthoringError> cutsceneValidation(CutsceneDTO cutscene) {
        ...
        if (cutscene.id == null) cutsceneErrors.add(new AuthoringError(
                "CUTSCENE_ID_NULL", "doc.cutscene", "Cutscene id must not be null"));

        if (cutscene.beats == null) cutsceneErrors.add(new AuthoringError(
                "CUTSCENE_BEATS_NULL", "doc.cutscene", "Beats must not be null"));
        ...
        ValidationCtx validationCtx = new ValidationCtx(characterIds, backgroundIds, soundIds, beatIds, characterPoses);
     ...
     * <ul>
     *     <li>{@link BeatDTO#advance} must not be null</li>
     *     <li>The {@link AdvanceDTO#mode} from {@link BeatDTO#advance} must not be null</li>
     *     <li>
     *         If {@link AdvanceDTO#mode} is {@code "auto"} or {@code "input"}, {@link AdvanceDTO#delay} and
     *         {@link AdvanceDTO#signalKey} must be null
     *     </li>
     *     <li>
     *         If {@link AdvanceDTO#mode} is {@code "auto_delay"}, {@link AdvanceDTO#delay} must not be null and
     *         {@link AdvanceDTO#signalKey} must be null
     *     </li>
     *     <li>
     *         If {@link AdvanceDTO#mode} is {@code "signal"}, {@link AdvanceDTO#delay} must be null, and
     *         {@link AdvanceDTO#signalKey} must not be null
     *     </li>
    ...
    private List<AuthoringError> beatValidation(BeatDTO beat, ValidationCtx validationCtx) {
        ...
        if (beat.advance == null) beatErrors.add(new AuthoringError(
                "BEAT_ADVANCE_NULL", "doc.cutscene.beats." + beat.id,
                "Beat advance must not be null"));

        else switch(beat.advance.mode) {
            case null:
                beatErrors.add(new AuthoringError(
                        "BEAT_ADVANCE_MODE_NULL", "doc.cutscene.beats." + beat.id,
                        "Beat advance mode must not be null"));
                break;
            case "auto":
            case "input":
                if (beat.advance.delay != null || beat.advance.signalKey != null) beatErrors.add(
                        new AuthoringError("BEAT_ADVANCE_MODE_AUTO_UNEXPECTED",
                                "doc.cutscene.beats." + beat.id, "Unexpected value in auto advance"));
                break;
            case "auto_delay":
                if (beat.advance.delay == null || beat.advance.signalKey != null) beatErrors.add(
                        new AuthoringError("BEAT_ADVANCE_MODE_AUTO_DELAY_INVALID",
                                "doc.cutscene.beats." + beat.id, "Invalid data for auto delay advance"));
                break;
            case "signal":
                if (beat.advance.delay != null || beat.advance.signalKey == null) beatErrors.add(
                        new AuthoringError("BEAT_ADVANCE_MODE_SIGNAL_INVALID",
                                "doc.cutscene.beats." + beat.id, "Invalid data for signal advance"));
                break;
            default:
        ...
        if (beat.actions == null || beat.actions.isEmpty()) {
            beatErrors.add(new AuthoringError("BEAT_ACTIONS_NULL", "doc.cutscene.beats." + beat.id,
