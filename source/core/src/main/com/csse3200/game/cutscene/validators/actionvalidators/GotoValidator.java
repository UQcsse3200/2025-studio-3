 ...
 * <ul>
 *     <li>{@code cutsceneId} must be a string</li>
 *     <li>{@code beatId} must be a string</li>
 *     <li>If {@code cutsceneId == "current"} then {@code beatId} must reference a valid {@link Beat}</li>
 * </ul>
 *
...
public class GotoValidator implements ActionValidator {
    ...
    public List<AuthoringError> validate(String beatId, ActionDTO action, ValidationCtx context) {
        ...
        errors.addAll(ValidatorUtils.validateString(action.fields.get("cutsceneId"), "GOTO_CUTSCENE_ID", path));

        List<AuthoringError> beatIdErrors = ValidatorUtils.validateString(action.fields.get("beatId"),
                "GOTO_CUTSCENE_BEAT_ID", path);
            ...
            if (action.fields.get("cutsceneId").equals("current")) {
                if (beatIdErrors.isEmpty()) {
                    ...
                    if (!context.beatIds().contains(gotoBeatId)) {
                        beatIdErrors.add(new AuthoringError("ACTION_GOTO_BEAT_ID_INVALID", path,
