 ...
 *
 * Validates audio stop event against the following conditions:
 * <ul>
 *     <li>{@code bus} must be a string</li>
 *     <li>{@code bus} must equal "music"</li>
 *     <li>{@code fadeMs} must be an integer</li>
 *     <li>{@code fadeMs} must be {@code >= 0}</li>
...
public class AudioStopValidator implements ActionValidator {
    ...
    public List<AuthoringError> validate(String beatId, ActionDTO action, ValidationCtx context) {
        ...
        if (busObject instanceof String) {
            ...
            if (!bus.equals("music")) {
                errors.add(new AuthoringError("ACTION_AUDIO_STOP_BUS_INVALID", path,
        ...
        if (action.fields.get("fadeMs") instanceof Long) {
            if ((Long)action.fields.get("fadeMs") < 0) {
                errors.add(new AuthoringError("ACTION_AUDIO_STOP_FADE_INVALID", path,
                        "FadeMs must be greater or equal to 0"));
            }
        }
