 ...
 * validate} method.
 *
 * <p>Validates choice events against the following conditions:
 *
 * <ul>
 *   <li>{@code prompt} must be a string
 *   <li>{@code choices} must be a list
 *   <li>Each element of {@code choices} must be a map
 *   <li>Each entry in each choice map must have a {@link String} key and {@link String} value
 *   <li>No key or value can be empty or null
 *   <li>The choice field {@code entryBeatId} must be a valid beat id (only checked for {@code
 *       "current"} {@code cutsceneId})
...
public class ChoiceValidator implements ActionValidator {
   ...
   * @return A list of {@link AuthoringError} for each rule infraction
   */
  private List<AuthoringError> validateChoice(
      Map<?, ?> choice, String path, ValidationCtx context) {
    ...
    for (Map.Entry<?, ?> entry : choice.entrySet()) {
      if (!(entry.getKey() instanceof String) || !(entry.getValue() instanceof String)) {
        ...
        if (key.isEmpty() || value.isEmpty()) {
          choiceErrors.add(
              new AuthoringError(
                  "ACTION_CHOICE_EMPTY_STRING", path, "A key or value in the choice is empty"));
        } else if (key.equals("entryBeatId") && !context.beatIds().contains(value)) {
          choiceErrors.add(
              new AuthoringError(
                  "ACTION_CHOICE_INVALID_BEAT_ID",
                  path,
    ...
    if (!keys.containsAll(List.of("type", "line", "cutsceneId", "entryBeatId"))) {
      choiceErrors.add(
          new AuthoringError(
              "ACTION_CHOICE_MISSING_PARAMETERS",
              path,
