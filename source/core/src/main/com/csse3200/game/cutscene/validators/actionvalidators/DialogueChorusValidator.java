 ...
 * validate} method.
 *
 * <p>Validates dialogue chorus events against the following conditions:
 *
 * <ul>
 *   <li>{@code characterIds} must be a list
 *   <li>Each {@code characterIds} element that is a valid string must also correspond to a valid
 *       character (the string is a {@code characterId})
...
public class DialogueChorusValidator implements ActionValidator {
  ...
  public List<AuthoringError> validate(String beatId, ActionDTO action, ValidationCtx context) {
    ...
    if (characterIdsObject instanceof List<?>) {
      ...
      errors.add(
          new AuthoringError(
              "DIALOGUE_CHORUS_CHARACTERIDS_INVALID",
              path,
