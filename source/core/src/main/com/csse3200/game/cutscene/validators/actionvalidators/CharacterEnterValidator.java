 ...
 *
 * Validates character enter events against the following conditions:
 * <ul>
 *     <li>{@code characterId} must be a string</li>
 *     <li>{@code characterId} must correspond to a valid character</li>
 *     <li>{@code pose} must be a string</li>
 *     <li>{@code pose} must correspond to a valid pose in the {@code character} from the {@code characterId}</li>
 *     <li>{@code position} must be a string</li>
 *     <li>{@code position} must be either {@code "left"} or {@code "right"}</li>
...
public class CharacterEnterValidator implements ActionValidator {
    ...
    public List<AuthoringError> validate(String beatId, ActionDTO action, ValidationCtx context) {
        ...
        List<AuthoringError> poseErrors = ValidatorUtils.validateString(action.fields.get("pose"), "pose", path);
        ...
        if (characterIdErrors.isEmpty()) {
            ...
            if (poseErrors.isEmpty()) {
                ...
                if (!context.characterPoses().get(characterId).contains(pose)) {
                    errors.add(new AuthoringError("ACTION_CHARACTER_POSE_INVALID", path,
                            "Pose " + pose + " does not exist for the character " + characterId));
                }
            }
        }

        List<AuthoringError> positionErrors = ValidatorUtils.validateString(action.fields.get("position"),
                "position", path);
        errors.addAll(positionErrors);

        if (positionErrors.isEmpty()) {
            String position = (String) action.fields.get("position");

            if (!position.equals("left") && !position.equals("right")) {
                errors.add(new AuthoringError("ACTION_CHARACTER_POSITION_INVALID", path,
