 ...
 * validate} method.
 *
 * <p>Validatess audio set events against the following conditions:
 *
 * <ul>
 *   <li>{@code bus} is a valid string
 *   <li>{@code bus} must be equal to "music"
 *   <li>{@code volume} must be a double or float
 *   <li>{@code volume} &isin; [0.0, 1.0]
 * </ul>
 */
  ...
  public List<AuthoringError> validate(String beatId, ActionDTO action, ValidationCtx context) {
    ...
    if (busObject instanceof String) {
      ...
      if (bus.equals("music")) {
        ...
        errors.add(
            new AuthoringError(
                "ACTION_AUDIO_SET_BUS_INVALID", path, "Bus for audio set must be only \"music\""));
      }
    }
