 ...
 * ("kill") robot enemies currently present. Design rationale: - LevelGameArea exposes spawnRobot(x,
 * y, type) but does NOT expose a public enemy list, nor does it return the spawned Entity.
 * Therefore, precise "remove exactly what we spawned" bookkeeping is not possible here without
 * changing other files. - For DESTROY_ENEMY we use a best-effort approach: iterate over all
...
public final class SlotEffect {
  ...
  private static final class DraggableCard extends Image {
    ...
    private void tryPlaceViaOfficialPipeline() {
      ...
      // 1) Card center in stage coords -> screen pixels (LevelGameArea.stageToWorld expects screen
      ...
      // 4) Build a minimal "selected-card" entity that only carries DeckInputComponent with
      // supplier
      Entity selected =
          new Entity()
              .addComponent(
      ...
      if (!occupiedBefore && occupiedAfter) {
        remove();
        logger.info(
            "[SlotEffect][Card] Placed slingshooter at row={}, col={}, pos={}", row, col, pos);
      } else {
        logger.info(
            "[SlotEffect][Card] Placement did not take effect (occupied or validation failed)");
      }
    }
  }

  /**
   * On triple-hit, create a draggable card that drops from the reels area to the bottom. After the
   * drop it can be dragged; on release inside a valid grid cell, placement goes through
