...
import org.slf4j.LoggerFactory;
    ...
    /**
     * Periodically spawns "sunlight" currency at random positions on the screen. The spawn
     * interval, sun value, texture path and motion parameters are configurable. Actions registered
     * on the Stage are removed in {@link #dispose()} to avoid leaks.
     */
    public class CurrencyGeneratorComponent extends Component {
         ...
         * -> sun falls 25% of the screen height each second
         */
        ...
        public void create() {
            super.create();
            Stage stage = ServiceLocator.getRenderService() != null
            ...
            generatorAction = Actions.forever(Actions.sequence(
        ...
        public void spawnSunAt(float targetX, float targetY) {
            ...
            sun.addAction(Actions.sequence(
                    ...
                    Actions.parallel(
                            ...
                            Actions.scaleTo(0.85f, 0.85f, 0.25f)
                    ),
                    Actions.run(() -> {
                        if (sun.hasParent()) {
                            sun.clearActions();
                            sun.clearListeners();
                            sun.remove();
                            logger.debug("Sun expired without being collected at ({}, {})", targetX, targetY);
                        }
                    })
            ));
        }
