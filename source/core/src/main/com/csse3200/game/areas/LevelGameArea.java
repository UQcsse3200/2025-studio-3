...
public void spawnBossProjectile(Entity boss){
    ...
    public void spawnUnit(int position) {
        ...
        // The "fire" listener is now separate and ONLY handles firing.
        newEntity.getEvents().addListener("fire", () -> {
            spawnProjectile(entityPos);
            newEntity.getEvents().trigger("attackStart");
            // Corrected log message for clarity
            logger.info("Unit at position {} fired", position);
        });

        // The "death" listeners are now added only ONCE when the unit is created.
        newEntity.getEvents().addListener("defenceDeath", () -> {
        ...
        newEntity.getEvents().addListener(ENTITY_DEATH_EVENT, () -> {
            requestDespawn(newEntity);
            // This is for enemies, but defensively adding for all entities is fine
