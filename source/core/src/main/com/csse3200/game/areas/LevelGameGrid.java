...
public class LevelGameGrid {
    ...
    /**
     * Sets a specified (row, col) position on the grid to a provided tile entity. setTile differs from addTile as
     * setTile determines the index using the row and col values.
    ...
    public Entity getTileFromXY(float x, float y) {
        for (Entity tile : grid_data) {
            boolean status = tile.getComponent(TileHitboxComponent.class).inTileHitbox(new GridPoint2((int) x, (int) y));
            if (status) {
                return tile;
            }
        }
