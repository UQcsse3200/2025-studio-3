...
public class WorldMapScreen extends BaseScreen {
    ...
    // Dev: press M to play Simar's Level 3 intro cutscene anywhere on the map.
    // Update the path if your JSON lives somewhere else (e.g., "data/cutscene3.json").
    ...
    private void handleDevHotkey() {
        if (Gdx.input.isKeyJustPressed(Input.Keys.N)) {
            ...
            ServiceLocator.getCutsceneService().playCutscene(
                    DEV_CUTSCENE_PATH, // bare name
                    name -> { /* callback fires immediately on start in your service */ }
            );
        }
    }
