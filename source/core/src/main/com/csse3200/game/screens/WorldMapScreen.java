...
public class WorldMapScreen implements Screen {
    ...
    // Logical map size (we render the map texture to this virtual size)
    ...
    public void show() {
        ...
        // Back button (image) with hover pop and glow
        ...
        backBtn.addListener(new InputListener() {
            @Override public void enter(InputEvent e, float x, float y, int pointer, Actor from) {
                hoveringBack = true; backBtn.setScale(1.07f);
            }
            @Override public void exit(InputEvent e, float x, float y, int pointer, Actor to) {
                hoveringBack = false; backBtn.setScale(1f);
            }
        });

        // Shop button (no Persistence)
        ...
        // Inventory button (no Persistence)
    ...
    public void render(float delta) {
        ...
        for (Node node : nodes) {
            ...
            if (hovering || near) {
                if (node.unlocked) {
                    worldShapes.setColor(1f, 1f, 0f, 0.5f); // yellow for unlocked
                } else {
                    worldShapes.setColor(0.7f, 0.7f, 0.7f, 0.5f); // gray for locked
                }
        ...
        // -------- WORLD: nodes (with pop on hover) + prompts + player --------
        batch.begin();
        nearbyNode = null;
        for (Node node : nodes) {
            Texture nodeTex;
    ...
    private void handleInput(float delta) {
        ...
        // Interact (sequential gating, like your Overhaul-1)
    ...
    // Convert mouse (screen) → world coords
    ...
    private boolean isMouseOverNode(Vector2 mouseWorld, float nodeX, float nodeY, float size) {
        return mouseWorld.x >= nodeX && mouseWorld.x <= nodeX + size
                && mouseWorld.y >= nodeY && mouseWorld.y <= nodeY + size;
    }
    ...
    private void clampCamera() {
        ...
        float minX = w / 2f;
        float maxX = worldWidth - w / 2f;
        float minY = h / 2f;
        float maxY = worldHeight - h / 2f;
