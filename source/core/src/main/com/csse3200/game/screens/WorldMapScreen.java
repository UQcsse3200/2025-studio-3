...
public class WorldMapScreen implements Screen {
    ...
    private void handleInput(float delta) {
        ...
        if (Gdx.input.isButtonJustPressed(Input.Buttons.LEFT)) {
            ...
            if (backBtnBounds.contains(mx, my)) {
                logger.info("⬅️ Back to Main Menu");
        ...
        if (nearbyNode != null && Gdx.input.isKeyJustPressed(Input.Keys.E)) {
            if (nearbyNode.level == 1) {
                ...
                // TODO: Hook this into MainGameScreen when level ends
                // returnToWorldMap(nearbyNode);
            } else {
    ...
    /**
     * Called when a level is finished (win or lose).
     * This returns the player back to the World Map and marks the node completed.
     */
    public void returnToWorldMap(Node node) {
        logger.info("🏁 Level {} finished. Returning to World Map.", node.level);

        node.completed = true;   // mark node as done
