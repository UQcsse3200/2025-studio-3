...
public class TileTest {
    ...
    void beforeEach() {
        ...
        com.csse3200.game.rendering.RenderService renderService = mock(com.csse3200.game.rendering.RenderService.class);
        when(renderService.getStage()).thenReturn(stage);
        ServiceLocator.registerRenderService(renderService);

        // creates mock resource service
        com.csse3200.game.services.ResourceService resourceService = mock(com.csse3200.game.services.ResourceService.class);
        lenient().when(resourceService.getAsset(eq("images/ghost_1.png"), eq(Texture.class))).thenReturn(ghost1);
        lenient().when(resourceService.getAsset(eq("images/ghost_king.png"), eq(Texture.class))).thenReturn(ghost2);
        lenient().when(resourceService.getAsset(eq("images/selected_star.png"), eq(Texture.class))).thenReturn(star);
    ...
    public void shouldAddUnit() {
        ...
        //because the selected unit is duplicated when placed on tile, so cant test that the both entities are the same
        assert(tileStorageComponent.getTileUnit().getComponent(TextureRenderComponent.class).getTexture()
    ...
    public void shouldntAddUnit() {
        ...
        TileStorageComponent tileStorageComponent = tile.getComponent(TileStorageComponent.class);
        tileStorageComponent.triggerSpawnUnit();
        Entity newSelected = new Entity()
                .addComponent(new TextureRenderComponent("images/ghost_king.png"));
        levelGameArea.setSelectedUnit(newSelected);
        tileStorageComponent.triggerSpawnUnit();
        // checks if the tile unit has not been replaced with new unit if there was already a unit placed
        assert(tileStorageComponent.getTileUnit().getComponent(TextureRenderComponent.class).getTexture()
                == selected.getComponent(TextureRenderComponent.class).getTexture());
        assert(tileStorageComponent.getTileUnit().getComponent(TextureRenderComponent.class).getTexture()
