...
class WaveServiceTest {
    ...
    void initialiseNewWave_entersPreparation_and_firesEvents() {
        ...
        svc.setWaveEventListener(new WaveService.WaveEventListener() {
            @Override public void onPreparationPhaseStarted(int waveNumber) { if (waveNumber == 1) prep[0] = true; }
            @Override public void onWaveChanged(int waveNumber) { if (waveNumber == 1) changed[0] = true; }
    ...
    void update_transitionsFromPreparationToActive_and_firesStartEvent() {
        ...
        svc.setWaveEventListener(new WaveService.WaveEventListener() {
            @Override public void onPreparationPhaseStarted(int waveNumber) {}
            @Override public void onWaveChanged(int waveNumber) {}
            @Override public void onWaveStarted(int waveNumber) { started[0] = (waveNumber == 1); }
        });
    ...
    void bossQueue_spawnsBossOnUpdate_forFirstThreeWaves() {
        ...
        svc.setEnemySpawnCallback(new WaveService.EnemySpawnCallback() {
            ...
            @Override public void spawnBoss(int row, BossFactory.BossTypes bossType) { bosses.add(bossType); }
        });
    ...
    void spawnEnemy_triggersCallback_whenWaveActive() {
        ...
        svc.setEnemySpawnCallback(new WaveService.EnemySpawnCallback() {
            @Override public void spawnEnemy(int col, int row, RobotType robotType) { spawned[0]++; }
