...
class GunnerAttackTaskTest {
    ...
    void setup() {
        ...
        when(entityService.getEntities()).thenReturn(new com.badlogic.gdx.utils.Array<>(new Entity[]{defense}));
    }

    @Test
    void lowPriorityWhenTargetNot() {
        // no visible target
        when(ServiceLocator.getEntityService().getEntities()).thenReturn(new com.badlogic.gdx.utils.Array<>());
    ...
    void fireOnceCooldownReached() {
        ...
        GunnerAttackTask task = new GunnerAttackTask(10f, (short) 1) {
            @Override
