...
class TargetDetectionTasksTest {
  ...
  void constructorTest() {
    TestTargetDetectionTasks task =
        new TestTargetDetectionTasks(5f, TargetDetectionTasks.AttackDirection.LEFT);
  ...
  void getPriorityActiveTest() {
    try {
      java.lang.reflect.Field status =
          TargetDetectionTasks.class.getSuperclass().getDeclaredField("status");
  ...
  void getAllTargetsTest() {
    ...
    when(entity4.getComponent(CombatStatsComponent.class))
        .thenReturn(null); // no CombatStatsComponent
    when(hitboxEnemy.getLayer()).thenReturn(PhysicsLayer.ENEMY);
