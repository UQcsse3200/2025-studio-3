...
class LaneRunnerPlayerFactoryTest{
    ...
    void testCreatePlayer(){
        ...
        assertNotNull(player.getComponent(TextureRenderComponent.class), "Player should have TextureRenderComponent");
        assertNotNull(player.getComponent(MiniGameInputComponent.class), "Player should have MiniGameInputComponent");

        TextureRenderComponent textureRenderComponent = player.getComponent(TextureRenderComponent.class);
        assertNotNull(textureRenderComponent, "TextureRenderComponent should exist");

        assertEquals(LaneConfig.OBSTACLE_WIDTH, player.getScale().x, 0.001f, "Player scale X should match OBSTACLE_WIDTH");
        assertEquals(LaneConfig.OBSTACLE_HEIGHT, player.getScale().y, 0.001f, "Player scale Y should match OBSTACLE_HEIGHT");

        assertEquals(5.0f, player.getPosition().x, 0.001f, "Player X position should be the centre of the starting lane");
        assertEquals(LaneConfig.PLAYER_Y, player.getPosition().y, 0.001f, "Player Y position should match PLAYER_Y");
