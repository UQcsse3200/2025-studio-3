...
public class DefenceFactoryTest {
    ...
    void testGetTaskComponentLeft() {
        ...
        try {
            java.lang.reflect.Field priorityTasksField = AITaskComponent.class.getDeclaredField("priorityTasks");
            ...
            java.lang.reflect.Field attackRange = AttackTask.class.getSuperclass().getDeclaredField("attackRange");
            attackRange.setAccessible(true);
            java.lang.reflect.Field attackDirection = AttackTask.class.getSuperclass().getDeclaredField("direction");
            ...
            java.lang.reflect.Field idleRange = AttackTask.class.getSuperclass().getDeclaredField("attackRange");
            idleRange.setAccessible(true);
            java.lang.reflect.Field idleDirection = AttackTask.class.getSuperclass().getDeclaredField("direction");
    ...
    void testCreateBaseDefender() {
        ...
        assertNotNull(baseDefender.getComponent(ColliderComponent.class), "ColliderComponent should exist");
        assertNotNull(baseDefender.getComponent(PhysicsComponent.class), "PhysicsComponent should exist");
        assertNotNull(baseDefender.getComponent(HitboxComponent.class), "HitboxComponent should exist");
        assertNotNull(baseDefender.getComponent(HitMarkerComponent.class), "HitMarkerComponent should exist");

        PhysicsComponent physics = baseDefender.getComponent(PhysicsComponent.class);
        assertEquals(BodyDef.BodyType.StaticBody, physics.getBody().getType(), "Body type should be StaticBody");

        ColliderComponent collider = baseDefender.getComponent(ColliderComponent.class);
        short expectedFilter = (short) (PhysicsLayer.DEFAULT | PhysicsLayer.OBSTACLE | PhysicsLayer.ENEMY);
        assertEquals(PhysicsLayer.NPC, collider.getFixture().getFilterData().categoryBits, "Collision layer should be NPC");
        assertEquals(expectedFilter, collider.getFixture().getFilterData().maskBits, "Collision mask should be correct");
