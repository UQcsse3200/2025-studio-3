...
/** Stores data for menu sprites also facilitates activating a sprite */
public class MenuSpriteData {
   ...
   */
  private static <E extends Enum<E>> void dynamicMenuEnter(
      DynamicMenuSpriteScreen<E> dynamicMenuSpriteScreen, Enum<?> id) {
   ...
   */
  public void enter(GdxGame game)
    ...
    if (MenuSpriteScreen.class.isAssignableFrom(screenClass)) {
      // static
      MenuSpriteScreen screen =
          (MenuSpriteScreen) screenClass.getDeclaredConstructor(GdxGame.class).newInstance(game);
      ...
      // dynamic
      DynamicMenuSpriteScreen<?> screen =
          (DynamicMenuSpriteScreen<?>)
              screenClass.getDeclaredConstructor(GdxGame.class).newInstance(game);
      Screen currentScreen = game.getScreen();
      if (currentScreen != null) {
        currentScreen.dispose();
      }
